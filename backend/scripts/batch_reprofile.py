#!/usr/bin/env python3
"""Batch re-profile unified_profiles via IntentProfiler + Ollama.

Usage:
    python scripts/batch_reprofile.py [--limit 10] [--concurrency 2]
"""
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch re-profile unified profiles.")
    parser.add_argument("--limit", type=int, default=0, help="Max profiles to process (0 = all)")
    parser.add_argument("--concurrency", type=int, default=CONCURRENCY, help="Parallel workers")
    return parser.parse_args(argv)


async def get_all_profile_uids(limit: int = 0) -> List[str]:
...
async def run(limit: int = 0, concurrency: int = CONCURRENCY) -> None:
    ...
    uids = await get_all_profile_uids(limit=limit)
    total = len(uids)
