...
async def consume_loop(
    ...
    try:
        async for msg in consumer:
            ...
            # Write to MongoDB raw_events (for LiveFeed dashboard)
            if raw_events_coll is not None:
                try:
                    await raw_events_coll.insert_one(validated.model_dump(mode="json"))
                    stats["mongo_written"] = stats.get("mongo_written", 0) + 1
                except Exception as exc:
                    logger.error("mongo_write_error", error=str(exc), event=validated.session_id)
...
async def main_async() -> None:
    ...
    # MongoDB raw_events connection (for LiveFeed dashboard)
    raw_events_coll = None
    mongo_raw = None
    if cfg.mongo_uri and AsyncIOMotorClient is not None:
        try:
            mongo_raw = AsyncIOMotorClient(cfg.mongo_uri)
            raw_events_coll = mongo_raw[MONGODB_DATABASE][RAW_EVENTS_COLLECTION]
            logger.info("raw_events_connected", db=MONGODB_DATABASE, coll=RAW_EVENTS_COLLECTION)
        except Exception as exc:
            logger.error("raw_events_connection_failed", error=str(exc))

    consumer_task = asyncio.create_task(
        consume_loop(cfg=cfg, uid_queue=uid_queue, s3_writer=s3_writer, raw_events_coll=raw_events_coll)
    )
