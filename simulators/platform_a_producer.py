#!/usr/bin/env python3
"""Platform A event producer for the CDP Agentic Ad Engine.

Reads the Platform A clickstream CSV and emits each row as a JSON
event to the Redpanda (Kafka-compatible) topic ``platform-a-events``.

Usage:
    python simulators/platform_a_producer.py \\
        --csv data/platform_a_events.csv \\
...
logger = get_logger("platform_a_producer")
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Platform A Event Producer — emits CSV rows to Redpanda topic.",
    )
    parser.add_argument(
        "--csv", type=str, default="data/platform_a_events.csv",
    ...
    parser.add_argument(
        "--topic", type=str, default="platform-a-events",
        help="Kafka topic name (default: platform-a-events)",
    )
    parser.add_argument(
...
def enrich_row(row: Dict[str, Any]) -> bytes:
    """Convert a CSV row dict into a JSON byte-string matching the
    :class:`common.schemas.ClickstreamEvent` schema.

    Fields are already in the expected format from the CSV;
    this function normalises types and adds the event timestamp.
    """
    # Build the ClickstreamEvent-like JSON payload
    payload = {
        "platform": "A",
...
def _throttle_delay(throttle: int) -> float:
    """Return the per-event sleep in seconds to achieve *throttle* events/sec."""
    if throttle <= 0:
        return 0.0
    return 1.0 / throttle
...
async def main_async() -> None:
    args = parse_args()
    setup_logging(service_name="platform-a-producer")
